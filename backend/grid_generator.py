...
def generate_grid_analysis():
    ...
    print("--- Grid Analiz Motoru Başlatılıyor ---")
    
    # 1. Mevcut Analizleri Temizle (Yeniden oluşturuyoruz)
    db.query(GridAnalysis).delete()
    db.commit()
    print("Eski harita verileri temizlendi.")

    # 2. Tüm benzersiz koordinatları bul
    locations = db.query(
        WeatherData.latitude, 
        WeatherData.longitude
    ).group_by(WeatherData.latitude, WeatherData.longitude).all()
    
    print(f"Toplam {len(locations)} farklı lokasyon analiz edilecek...")
    ...
    for idx, (lat, lon) in enumerate(locations):
        ...
        # --- GÜNEŞ ANALİZİ (Solar Grid) ---
        # MJ -> kWh (1 kWh = 3.6 MJ)
        avg_rad_kwh = (stats.avg_rad / 3.6) if stats.avg_rad else 0
        
        # Skorlama (0-100 arası): Türkiye'de ortalama 3.8 - 5.5 kWh arasıdır.
        # 3.0 altı = 0 puan, 6.0 üstü = 100 puan diyelim.
        ...
        solar_grid = GridAnalysis(
            ...
            logistics_score=1.0 # Şimdilik sabit
        )
        grid_objects.append(solar_grid)

        # --- RÜZGAR ANALİZİ (Wind Grid) ---
        avg_wind = stats.avg_wind if stats.avg_wind else 0
        
        # Skorlama: 3 m/s altı çöp, 9 m/s üstü harika.
        ...
        wind_grid = GridAnalysis(
            ...
            longitude=lon,
            type="Wind",
            annual_potential_kwh_m2=0, # Rüzgar için bu alan boş kalabilir
        ...
        if idx % 50 == 0:
            print(f"İşlenen: {idx}/{len(locations)}")
    ...
    print(f"🎉 Analiz Tamamlandı! {len(grid_objects)} adet grid hücresi oluşturuldu.")
