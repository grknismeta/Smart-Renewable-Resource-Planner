...
# ─── CO₂ Index (M-G.3, Mauna Loa proxy) ──────────────────────────────────────
#
# Atmospheric CO₂ aylık değeri — deterministik global iklim sinyali.
# NOAA Mauna Loa verisi (yıllık ortalama):
#   1990≈354, 2000≈369, 2010≈390, 2020≈414, 2024≈422 ppm
# Quadratic fit (ivmeyi yakalar):
#   ppm(t) = 354 + 1.7·t + 0.012·t²  ,  t = year - 1990
# Seasonal modülasyon: Apr-May peak, Aug-Sep min (kuzey yarımküre fotosentez).
# Forecast horizon'da deterministik artar → SARIMAX exog → drift güçlenir.

def get_co2_ppm(year: int, month: int = 6) -> float:
    """Aylık atmospheric CO₂ ppm (Mauna Loa-style proxy)."""
    t = year - 1990 + (month - 1) / 12.0
    base = 354.0 + 1.7 * t + 0.012 * t * t
    seasonal = 2.5 * math.sin(2 * math.pi * (month - 4.5) / 12.0)
...
# ─── 20 yıllık aylık iklim (M-E.2) — en uzun seri kaynağı ────────────────────

# metric → monthly_climate kolonu
_MONTHLY_CLIMATE_COLS = {
    "wind": "wind_speed_mean",
    "irradiance": "shortwave_radiation_sum",
    "sunshine": "shortwave_radiation_sum",  # radyasyon proxy
    "temperature": "temperature_mean",
    "temp": "temperature_mean",
...
def get_monthly_series_from_monthly_climate(
    province: str, metric: str,
) -> tuple[List[float], Optional["date"]]:  # type: ignore[name-defined]
    """`monthly_climate` (M-E.2) tablosundan il-bazlı aylık seri.

    En uzun kaynak: derin iller 2005-2026 (~257 ay), yerel iller 2015-2024.
    İlçe YOK (tablo il bazlı). Kolon NULL'sa (ör. yerel ilde precip/cloud) az
    satır döner → caller daily/climatology'ye fallback eder.

    Returns: (values, start_date). Seri <24 ay ise ([], None).
    """
    from datetime import date as _d
    from app.db.database import SystemSessionLocal
    from sqlalchemy import text
    ...
    col = _MONTHLY_CLIMATE_COLS.get(metric)
    if not col:
        return [], None
    variants = list(province_aliases(province))
    sql = text(f"""
        SELECT year, month, {col} AS v
        FROM monthly_climate
        WHERE province_name = ANY(:provs) AND {col} IS NOT NULL
        ORDER BY year, month
    """)
    with SystemSessionLocal() as db:
        rows = db.execute(sql, {"provs": variants}).fetchall()
    if not rows or len(rows) < 24:
        return [], None
    values = [float(r.v) for r in rows]
    start = _d(int(rows[0].year), int(rows[0].month), 1)
    return values, start


def get_monthly_series_best(
    province: str, district: Optional[str], metric: str,
) -> tuple[List[float], Optional["date"]]:  # type: ignore[name-defined]
    """En iyi (en uzun) aylık seri kaynağını seçer.

    İl-scope (district=None): monthly_climate (en uzun) → yoksa daily aggregate.
    İlçe-scope: monthly_climate il bazlı olduğundan sadece daily aggregate.

    Returns: (values, start_date) — ikisi de boş ise ([], None).
    """
    if district is None:
        mc_vals, mc_start = get_monthly_series_from_monthly_climate(province, metric)
        if mc_vals and len(mc_vals) >= 24:
            return mc_vals, mc_start
    return get_monthly_series_from_daily(province, district, metric)
