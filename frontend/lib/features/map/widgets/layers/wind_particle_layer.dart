  ...
  static _QualityParams fromQuality(WindParticleQuality q) {
    switch (q) {
      case WindParticleQuality.light:
        return const _QualityParams(particleCount: 800, fps: 20, trailLen: 6, maxAge: 120);
      case WindParticleQuality.balanced:
        return const _QualityParams(particleCount: 2000, fps: 30, trailLen: 8, maxAge: 160);
      case WindParticleQuality.heavy:
        return const _QualityParams(particleCount: 5000, fps: 60, trailLen: 12, maxAge: 200);
    }
  }
}
...
class _WindParticleLayerState extends State<WindParticleLayer>
  ...
  // Simülasyon zaman adımı (saniye/frame)
  ...
  void _tick() {
    for (final p in _particles) {
      final (u, v, _) = _interpolate(p.lat, p.lon);
    ...
    // setState yerine sadece repaint tetikle
    // CustomPainter shouldRepaint zaten her frame'de true dönecek
  }
...
class _WindParticlePainter extends CustomPainter {
  ...
  final int trailLen;
  final (double, double, double) Function(double, double) interpolate;
  ...
  void paint(Canvas canvas, Size size) {
    for (final p in particles) {
      ...
      final screenPos = camera.getOffsetFromOrigin(LatLng(p.lat, p.lon));

      // Trail'e ekle
      p.trail.addLast(screenPos);
      ...
      // Hız rengini IDW ile hesapla
      final (_, _, speed) = interpolate(p.lat, p.lon);
      final baseColor = _speedColor(speed);
